...
//     // Read the content of the Word document
//     const fileContent = await mammoth.extractRawText({
//       path: file.path,
//     });
...
//     // Insert data into the instruction table
//     const queryInstruction =
//       "INSERT INTO instruction (examId, instructionHeading, examName, documentName, instructionTable_Img) VALUES (?, ?, ?, ?, ?)";
//     const valuesInstruction = [
//       req.body.examId,
//       req.body.instructionHeading,
//       req.body.examName,
//       fileName,
//       images,  // Corrected variable name
//     ];
...
//     // Insert each point into the instructions_points table with the correct instructionId
//     const queryPoints =
//       "INSERT INTO instructions_points (examId, points, instructionId, instructionHeading_) VALUES (?, ?, ?, ?)";
//     for (const point of filteredPointsArray) {
//       console.log(
//         "Inserting point:",
//         point,
...
// // Express route for getting instruction image
// app.get("/getInstructionImage/:instructionId", async (req, res) => {
//   try {
...
//   } catch (error) {
//     console.error("Error fetching instruction image:", error);
//     res.status(500).json({
//       success: false,
//       message: "Failed to fetch instruction image.",
//       error: error.message,
//     });
//   }
// });

// Express route to get instruction image and points
...
async function getInstructionImage(instructionId) {
  try {
    const queryImage =
      "SELECT instructionTable_Img FROM instruction WHERE instructionId = ?";
    const [resultImage] = await db.query(queryImage, [instructionId]);
  ...
  try {
    ...
    for (let i = 0; i < images.length && i < textSections.length; i++) {
      if (textSections[i].startsWith(QUESTION_MARK)) {
        ...
        j++;
      } else if (j > 0 && j < OPTION_MARKERS.length + 1) {
        // Process option images
        const optionRecord = {
          option_img: images[i]
            .toString()
            .replace(new RegExp(`${OPTION_MARKERS[j - 1]}$`), ""),
          question_id: que_id,
          option_marker: OPTION_MARKERS[j - 1],
        };
        await insertRecord("options", optionRecord);
        j++;
      } else if (j === SOLN.length + 1) {
...
app.get(
  "/getSubjectData/:subjectId/:testCreationTableId/:sectionId",
  async (req, res) => {
    try {
      const subjectId = req.params.subjectId;
      const testCreationTableId = req.params.testCreationTableId;
      ...
      // Fetch document data based on subjectId, testCreationTableId, and sectionId
      const documentData =
        await dbHelper.getDocumentBySubjectAndTestCreationIdSectionId(
          subjectId,
          testCreationTableId,
          sectionId
        );
...
//_____________________________________________________Replace_And_Update________________________________

app.get("/examReplace_And_Update", async (req, res) => {
  try {
    const query = "SELECT * FROM exams";
    const [rows] = await db.query(query);
    res.json(rows);
  } catch (error) {
    console.error("Error  fetching examReplace_And_Update count:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
});

app.get("/coures_Replace_And_Update/:examId", async (req, res) => {
  const examId = req.params.examId;
  try {
    const [rows] = await db.query(
      `SELECT courseCreationId, courseName FROM course_creation_table WHERE examId = ?`,
      [examId]
    );

    res.json(rows);
  } catch (error) {
    console.error("Error fetching courses for examId", error);
    res.status(500).send("Internal Server Error");
  }
});

app.get("/test_Replace_And_Update/:courseCreationId", async (req, res) => {
  const courseCreationId = req.params.courseCreationId;
  try {
    const [rows] = await db.query(
      `SELECT courseCreationId, TestName FROM test_creation_table WHERE courseCreationId = ?`,
      [courseCreationId]
    );
    res.json(rows);
  } catch (error) {
...
app.get("/subject_Replace_And_Update/:courseCreationId", async (req, res) => {
  const { courseCreationId } = req.params;
  try {
    const [subjects] = await db.query(
      `
...
//_____________________________________________________ end ----Replace_And_Update________________________________
